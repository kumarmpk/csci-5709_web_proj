...
var endSprint = (req, res) => {
  ...
  let whereNewActive =
    "WHERE Sprints.order = ? and projectID = ? and isComplete = ?";
  let whereTaskActiveSprint =
    "WHERE sprintid = ? and projectid = ? and status = 1";
  ...
  let activeSprintOrder =
    "SELECT Sprints.order as Sprintorder from Sprints " + whereSprintID;
  let updateTasksSprint =
    "UPDATE Tasks SET sprintid = ? " + whereTaskActiveSprint;
